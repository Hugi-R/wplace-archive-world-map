...
pub fn init_img(x1: i64, y1: i64, x2: i64, y2: i64) -> PalettedImage {
    assert!(x2 >= x1 && y2 >= y1);
    ...
    PalettedImage { width: width as usize, height: height as usize, indices: vec![0u8; (width*height) as usize] }
}

#[cfg(feature = "native")]
pub fn native_screenshot(base_url: &str, version: &str, x1: i64, y1: i64, x2: i64, y2: i64) -> Result<PalettedImage, Box<dyn Error>> {
    let mut target = init_img(x1, y1, x2, y2);
